...
def _get_clap_score_class():
    global CLAPScore
    if CLAPScore is not None:
        return CLAPScore
    try:
        module = import_module("frechet_audio_distance")
    except Exception as e:
        raise ModuleNotFoundError(
            "frechet_audio_distance is not installed or could not be loaded. "
            "Please install it with `pip install frechet-audio-distance`."
        ) from e
    CLAPScore = module.CLAPScore
    return CLAPScore
...
def clap_score_setup(
    ...
    clap_score_class = _get_clap_score_class()

    clap = clap_score_class(
        ckpt_dir=ckpt_dir,
